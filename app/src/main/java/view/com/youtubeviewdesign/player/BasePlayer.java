...
public abstract class BasePlayer implements Player.EventListener, AudioManager.OnAudioFocusChangeListener {
    ...
    private void initExoPlayerCache() {
        if (cacheDataSourceFactory == null) {
            DefaultDataSourceFactory dataSourceFactory = new DefaultDataSourceFactory(context,USER_AGENT, bandwidthMeter);
    ...
    public void handleIntent(Bundle intent) {
        ...
        videoUrl = intent.getString(VIDEO_URL);
        videoTitle = intent.getString(VIDEO_TITLE);
        videoThumbnailUrl = intent.getString(VIDEO_THUMBNAIL_URL);
        videoStartPos = intent.getLong(START_POSITION, -1L);
        uploaderName = intent.getString(CHANNEL_NAME);
        setPlaybackSpeed(intent.getFloat(PLAYBACK_SPEED, getPlaybackSpeed()));
